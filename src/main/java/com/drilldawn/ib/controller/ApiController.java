...
package com.drilldawn.ib.controller;

import com.drilldawn.ib.client.*;
import com.drilldawn.ib.client.Types.*;
import com.drilldawn.ib.controller.ApiConnection.ILogger;
...
public class ApiController implements EWrapper {
    ...
    @Override
    public void updateMktDepthL2(int reqId, int position, String marketMaker, int operation, int side, double price, int size, boolean isSmartDepth) {
        IDeepMktDataHandler handler = m_deepMktDataMap.get(reqId);
        if (handler != null) {
    ...
    public void tickOptionComputation(int reqId, int tickType, int tickAttrib, double impliedVol, double delta, double optPrice, double pvDividend, double gamma, double vega, double theta, double undPrice) {
        ...
        if (handler != null) {
            ...
            logger.info(String.format("not handled %s %s %s %s %s %s %s %s %s %s", tickType, tickAttrib, impliedVol, delta, optPrice, pvDividend, gamma, vega, theta, undPrice));
        }
        recEOM();
    }
    ...
    @Override
    public void historicalData(int reqId, com.drilldawn.ib.client.Bar bar) {
    ...
    @Override
    public void historicalDataUpdate(int reqId, com.drilldawn.ib.client.Bar bar) {
